...
<?php if ( have_posts() ) :
	...
	if ( is_category() ) {
		...
		$archive_title = __('Category Archives:', 'iDiary') . ' <span>' . $post_path . '</span>';
		$post_path = $categories.$cat_parents.'<h1>'.$post_path.'</h1>';
	} elseif ( is_tag() ) {
		$post_path=single_tag_title( '', false );
		$archive_title = __('Tag Archives:', 'iDiary') . ' <span>' . $post_path . '</span>';
		...
		if(is_day()) {
			...
			$post_path = get_the_date( _x( 'Y', 'yearly archives date format', 'iDiary' ) );
		} else {
			$post_path = get_the_date( _x( 'F Y', 'monthly archives date format', 'iDiary' ) );
		}
		$archive_title = __('Date Archives:', 'iDiary') . ' <span>'. $post_path . '</span>';
	} elseif ( is_author() ) {
		$curauth = get_userdata_in_author_archive();
		$post_path=$curauth->display_name;
		$archive_title = sprintf(__('All posts by <span>%s</span>','iDiary'), $post_path);
		$post_path= sprintf(__('All posts by <h1>%s</h1>','iDiary'), $post_path);
	} else {
		$post_path=__('Blog Archives', 'iDiary');
		$archive_title = __('Blog Archives', 'iDiary');
	}
	if($paged && $paged > 1) $archive_title .= '<span class="archive_page"> - ' . sprintf(__('Page %s','iDiary'), $paged) . '</span>'; ?>
	<div class="post_path">
		<?php _e('You are here:', 'iDiary'); ?> <a class="first_home" rel="nofollow" title="<?php _e('Go to homepage', 'iDiary'); ?>" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php _e('Home', 'iDiary'); ?></a> &raquo; <?php echo $post_path; ?>
	</div>
	...
	elseif(function_exists('wp_pagenavi')) {
		...
		if ( $total_pages > 1 ) {
			echo '<div id="pagination">';
				posts_nav_link(' | ', __('&laquo; Previous page','iDiary'), __('Next page &raquo;','iDiary'));
			echo '</div>';
		}
	} ?>
...
<?php else: ?>
	<div class="post post_s">
		<h2 class="title title_s"><?php _e('Error 404 - Not Found', 'iDiary'); ?></h2>
		<div class="entry">
